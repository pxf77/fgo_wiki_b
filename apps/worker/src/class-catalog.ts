...
function releasedNpEventKeys(
  ...
  return new Set(
    manifest.events.flatMap((event) =>
      ...
      event.target.type === "noble_phantasm"
  ...
  return {
    ...
    overrides: {
      ...
      noblePhantasms: candidate.noblePhantasms.map((np, index) => ({
        ...
        color: np.color,
        scope: np.scope,
        ...(np.hitCount !== undefined
          ? { hitCount: np.hitCount }
          : {}),
        targetTraits: [],
        effects: [],
        strengthened: false,
...
export function buildCnClassCatalog(
  ...
  const passedByCollectionNo = new Map(
    releaseGate.passed.map(
      (entry) => [entry.collectionNo, entry] as const,
    ),
  );
  ...
  const catalogCandidates = candidates.map<CnClassCatalogCandidate>(
    (candidate) => {
      ...
      const noblePhantasms =
        candidate.noblePhantasms.map<CnClassCatalogNoblePhantasm>(
          (np) => {
            const curated = curatedByAtlasSourceId.get(np.sourceId);
            let strengtheningStatus: CatalogStrengtheningStatus =
              "not_strengthened";
            if (np.strengthened) {
              if (!releasePassed) {
                ...
                passed &&
                releasedEvents.has(
                  `${passed.servantId}:${curated.id}`,
                )
              ) {
                strengtheningStatus = "evidenced";
              } else if (curated && passed) {
    ...
    return [
      {
        ...
        atlasCurrentStrengthenedNps: atlasStrengthenedNps.filter(
          (np) => np.strengtheningStatus === "atlas_current",
        ).length,
  ...
  missingStrengtheningEvents.sort(
    (left, right) =>
      left.collectionNo - right.collectionNo ||
      left.atlasSourceId - right.atlasSourceId,
  );
  return {
    schemaVersion: 1,
    generatedAt,
...
function assertReleaseGateReport(
  value: unknown,
): asserts value is CnReleaseGateReport {
  if (
    typeof value !== "object" ||
    value === null ||
    Array.isArray(value)
  ...
  if (
    ...
    throw new TypeError(
      "CN release gate report passed/blocked arrays are required",
    );
  }
}
