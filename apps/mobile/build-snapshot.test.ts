...
async function createReviewedSnapshotFile(): Promise<{
  ...
  const directory = await mkdtemp(join(tmpdir(), "fgo-mobile-snapshot-"));
  const path = join(directory, "snapshot.json");
  const snapshot = structuredClone(bootstrapSnapshot);
  snapshot.metadata = {
    ...snapshot.metadata,
    datasetVersion: "2026-08-13-r1--rel-release-r1--str-strengthening-r1",
...
test("fails the mobile build when the generated snapshot is missing", async () => {
  const directory = await mkdtemp(join(tmpdir(), "fgo-mobile-missing-"));
  try {
...
test("allows Bootstrap data only through the explicit development flag", async () => {
  const directory = await mkdtemp(join(tmpdir(), "fgo-mobile-bootstrap-"));
  const path = join(directory, "snapshot.json");
  await writeFile(path, `${JSON.stringify(bootstrapSnapshot)}\n`, "utf8");
  try {
    await assert.rejects(
      loadMobileBuildSnapshot({ snapshotPath: path, repositoryRoot: directory }),
